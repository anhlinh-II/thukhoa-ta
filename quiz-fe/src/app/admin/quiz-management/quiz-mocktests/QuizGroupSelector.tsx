...
import { GroupType } from "@/share/services/quiz_group/models";

export function QuizGroupSelector({ form, initialValue }: { form: any; initialValue?: number }) {
     ...
     useEffect(() => {
          // load initial selected group if editing; also load programs
     ...
     const fetchGroups = (programId?: number, q?: string) => {
          ...
          // only MOCK_TEST groups
          filters.push({ field: 'groupType', operator: 'EQUALS', value: GroupType.MOCK_TEST });
          if (q) filters.push({ field: 'name', operator: 'CONTAINS', value: q });
          const req: PagingViewRequest = {
               skip: 0, take: 200, filter: filters.length ? JSON.stringify(filters) : ''
          };
               ...
               <Form.Item label="Quiz Group" name="quizGroup" rules={[{ required: true, message: 'Please select group ID' }]}>
                    <Select
                         ...
                         onSearch={(v) => {
                              ...
                              timerRef.current = window.setTimeout(() => fetchGroups(selectedProgram ?? undefined, v), 250);
                         }}
                         onChange={pick}
                         options={options}
                         loading={loading}
